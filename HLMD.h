...
#define HLMD_PRIMITIVE_CAT(head, tail, ...) \
    head##tail /* Generate a new token by directly concatenating the head and tail without unfolding. */
#define HLMD_CAT(head, tail) \
    HLMD_PRIMITIVE_CAT(head, tail) /* After unfolding, splice the head and tail to generate a new token. */
...
#define HLMD_CHECK(...)               \
    HLMD_EXPAND(HLMD_GET_SECOND_ARGS( \
        __VA_ARGS__, 0))   /* Expand and get the second argument, defaulting to 0 if not present. */
#define HLMD_PROBE(x) x, 1 /* Used for macro detection, expands to x, 1. */

#define HLMD_NOT(x)                \
    HLMD_CHECK(HLMD_PRIMITIVE_CAT( \
        __HLMD_NOT_, x)) /* Logical NOT, returns 1 if x is `0, false, NULL, nullptr`, otherwise 0. */
...
#define HLMD_BOOL(x) \
    HLMD_NOT(HLMD_CHECK(HLMD_CAT(__HLMD_NOT_, x))) /* Convert x to 1 (true) or 0 (false) for macro logic. */
#define HLMD_IF_ELSE(condition)                                    \
    HLMD_CAT(__HLMD_IF_, HLMD_NOT(HLMD_CHECK(HLMD_CAT(__HLMD_NOT_, \
                             condition)))) /* Macro-based if-else, expands to true/false branch based on condition. */
...
#define HLMD_IS_EMPTY(...) \
    HLMD_CHECK(            \
        HLMD_CAT(HLMD_PRIMITIVE_CAT(__HLMD_IS_EMPTY_, __VA_ARGS__), 0)) /* Check if the parameter pack is empty. */
#define __HLMD_IS_EMPTY_0 HLMD_PROBE()

#define HLMD_IS_PAREN(x, ...) \
    HLMD_CHECK(HLMD__VA_OPT__(, 0, ) __HLMD_IS_PAREN_PROBE x) /* Check if the argument is parenthesized. */
#define HLMD_REMOVE_PAREN(...)                                         \
    HLMD_IF_ELSE(HLMD_IS_PAREN(__VA_ARGS__))(HLMD_EXPAND __VA_ARGS__)( \
        __VA_ARGS__) /* Remove parentheses from the argument if present. */
#define HLMD_REMOVE_PAREN_ARGS(...)                                       \
    HLMD_IF_ELSE(HLMD_IS_PAREN(__VA_ARGS__))(HLMD_EMPTY_ARG __VA_ARGS__)( \
        __VA_ARGS__) /* Remove parentheses and parameters, if any. */
#define __HLMD_IS_PAREN_PROBE(...) HLMD_PROBE()

/* Simulate the '__VA_OPT__' behavior: expand to 'empty_tuple' if the parameter package is empty, or 'data_tuple'
...
#define HLMD_EVAL_MANUAL(level, ...) \
    HLMD_CAT(__HLMD_EVAL, level)(__VA_ARGS__) /* Manually specify the number of recursive macro expansion rounds. */
#define HLMD_EVAL_AUTO(...) \
    HLMD_EVAL_MANUAL(HLMD_EVAL_MAX_LEVEL, __VA_ARGS__) /* Automatically expand macros up to the maximum level. */
